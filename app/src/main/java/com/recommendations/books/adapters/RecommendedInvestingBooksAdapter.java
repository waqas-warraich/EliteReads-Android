...
public class RecommendedInvestingBooksAdapter extends FirestoreRecyclerAdapter<Book, RecommendedInvestingBooksAdapter.RecommendedInvestingBooksVH> {

    Context context;

    public RecommendedInvestingBooksAdapter(@NonNull FirestoreRecyclerOptions<Book> options) {
        super(options);
    }

    @Override
    protected void onBindViewHolder(@NonNull RecommendedInvestingBooksVH holder, int position, @NonNull Book model) {
    ...
    @NonNull
    @Override
    public RecommendedInvestingBooksVH onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {

        context = parent.getContext();
        View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_book,
                parent, false);

        return new RecommendedInvestingBooksVH(v);
    }

    public class RecommendedInvestingBooksVH extends RecyclerView.ViewHolder{

        private ImageView ivBookImage;
        ProgressBar progressBarBookImageLoader;


        public RecommendedInvestingBooksVH(@NonNull View itemView) {
            super(itemView);
